...
import { exportCanvasToPDF } from "./pdf-export.js";
  ...
  updateProperties(selected) {
    ...
    if (selected.type === "wall") {
      this.propertiesPanel.innerHTML = `
        ...
        <div class="prop-row">Start X <input id="prop-start-x" type="number" step="0.1" value="${selected.start.x}"></div>
        <div class="prop-row">Start Y <input id="prop-start-y" type="number" step="0.1" value="${selected.start.y}"></div>
        <div class="prop-row">Ende X <input id="prop-end-x" type="number" step="0.1" value="${selected.end.x}"></div>
        <div class="prop-row">Ende Y <input id="prop-end-y" type="number" step="0.1" value="${selected.end.y}"></div>
        <div class="prop-row">Stärke <input id="prop-thickness" type="number" min="0.05" step="0.01" value="${selected.thickness}"></div>
    ...
    if (selected.type === "object") {
      this.propertiesPanel.innerHTML = `
        ...
        <div class="prop-row">X <input id="prop-x" type="number" step="0.1" value="${selected.x}"></div>
        <div class="prop-row">Y <input id="prop-y" type="number" step="0.1" value="${selected.y}"></div>
        <div class="prop-row">Breite <input id="prop-width" type="number" min="0.05" step="0.05" value="${selected.width}"></div>
        <div class="prop-row">Höhe <input id="prop-height" type="number" min="0.05" step="0.05" value="${selected.height}"></div>
        <div class="prop-row">Rotation <input id="prop-rotation" type="number" step="0.1" value="${selected.rotation}"></div>
        <button id="apply-props">Übernehmen</button>
      `;
      document.getElementById("apply-props").onclick = () => {
    ...
    if (selected.type === "door" || selected.type === "window") {
      this.propertiesPanel.innerHTML = `
        <div class="prop-row"><span class="prop-label">Typ</span>: ${selected.type}</div>
        <div class="prop-row">Position t <input id="prop-t" type="number" min="0.05" max="0.95" step="0.01" value="${selected.positionT}"></div>
        <div class="prop-row">Breite <input id="prop-width" type="number" min="0.2" step="0.05" value="${selected.width}"></div>
        <div class="prop-row">Brüstung <input id="prop-sill" type="number" min="0" step="0.05" value="${selected.sillHeight}"></div>
        <div class="prop-row">Höhe <input id="prop-height" type="number" min="0.2" step="0.05" value="${selected.height}"></div>
        <button id="apply-props">Übernehmen</button>
      `;
      document.getElementById("apply-props").onclick = () => {
    ...
    if (selected.type === "dimension") {
      this.propertiesPanel.innerHTML = `
        <div class="prop-row"><span class="prop-label">Typ</span>: Bemaßung</div>
        <div class="prop-row">Start X <input id="prop-start-x" type="number" step="0.1" value="${selected.start.x}"></div>
        <div class="prop-row">Start Y <input id="prop-start-y" type="number" step="0.1" value="${selected.start.y}"></div>
        <div class="prop-row">Ende X <input id="prop-end-x" type="number" step="0.1" value="${selected.end.x}"></div>
        <div class="prop-row">Ende Y <input id="prop-end-y" type="number" step="0.1" value="${selected.end.y}"></div>
        <div class="prop-row">Offset <input id="prop-offset" type="number" step="0.05" value="${selected.offset ?? 0.25}"></div>
        <button id="apply-props">Übernehmen</button>
      `;
      document.getElementById("apply-props").onclick = () => {
