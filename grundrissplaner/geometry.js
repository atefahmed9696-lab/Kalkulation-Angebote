    ...
    for (let cx = 0; cx < cols; cx++) {
      ...
      while (queue.length) {
        ...
        const neighbors = [{
          nx: current.cx - 1,
          ny: current.cy,
          ok: current.cx > 0 && canMoveLeft(current.cx, current.cy)
        }, {
          nx: current.cx + 1,
          ny: current.cy,
          ok: current.cx < cols - 1 && canMoveRight(current.cx, current.cy)
        }, {
          nx: current.cx,
          ny: current.cy - 1,
          ok: current.cy > 0 && canMoveUp(current.cx, current.cy)
        }, {
          nx: current.cx,
          ny: current.cy + 1,
          ok: current.cy < rows - 1 && canMoveDown(current.cx, current.cy)
        }];
